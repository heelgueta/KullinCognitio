...
optionally splits articles into phrases, exports cleaned CSV + JSON metadata.
"""
...
def process():
    ...
    def generate():
        ...
        yield _sse({"type": "log", "level": "info",
                    "msg": f"{total} artículos. Limpiando con opciones: {opts}"})
        ...
        for i, row in enumerate(all_rows):
            ...
            for field in ("title", "body_text"):
                if field in cleaned:
                    cleaned[field] = clean_text(cleaned[field] or "", opts)

            if mode == "phrase":
                for phrase in split_sentences(cleaned.get("body_text", "")):
        ...
        yield _sse({"type": "log", "level": "ok",
                    "msg": f"✓ Listo — {len(out_rows)} filas exportadas → {out_name}"})
